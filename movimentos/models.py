...
class Chamado(models.Model):
    ...
    prioridade = models.CharField(
        max_length=15, verbose_name="Prioridade", choices=PRIORIDADE
    )
    atendente = models.ForeignKey(
        to="base.Usuario", verbose_name="Usuario", on_delete=models.DO_NOTHING
    )
    status = models.ForeignKey(
        to="cadastros.Status", verbose_name="Status", on_delete=models.DO_NOTHING
    )
    categoria = models.ForeignKey(
        to="cadastros.Categoria", verbose_name="Categoria", on_delete=models.DO_NOTHING
    )
    situacao = models.CharField(
        max_length=15, verbose_name="Situação", choices=SITUACAO
    )
    ...
    obs_fechamento = models.TextField(
        max_length=300,
        blank=True,
        null=True,
